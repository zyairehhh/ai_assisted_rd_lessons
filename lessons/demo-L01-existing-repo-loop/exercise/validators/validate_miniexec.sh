#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────
# Mini Query Executor — exercise validator
# ──────────────────────────────────────────────────────────────────
set -uo pipefail
...
echo "=== L01 Mini Query Executor validation ==="
echo ""

# 1. Unit tests (only miniexec tests)
echo "── Unit tests ──"
PYTHONPATH="$ROOT_DIR/src" python3 -m unittest \
    tests.test_miniexec_evaluator tests.test_miniexec_executor -v 2>&1 | tail -1
echo ""
if PYTHONPATH="$ROOT_DIR/src" python3 -m unittest \
    tests.test_miniexec_evaluator tests.test_miniexec_executor 2>/dev/null; then
...
if [ "$FAIL" -eq 0 ]; then
    echo ""
    echo "  ✓ L01 Mini Query Executor validation passed"
else
    echo ""
